...
void Game::update(float dt)
{
    ...
    case StateName::Menu:
        {
            if(std::shared_ptr<MenuState> menu = std::dynamic_pointer_cast<MenuState>(m_states.top()))
            {
                ...
                else if(menu->getPlay())
                {
                    menu->resetPlay();
                    m_states.push(
                        std::make_shared<SpaceshipSelectionState>(
                            ResourceManager::getTexture("background"), m_windowInfo, m_mouse
                        )
                    );
                }
                else if(glfwGetKey(m_windowInfo->window.get(), GLFW_KEY_A) == GLFW_PRESS)
                    m_states.push(
                        std::make_shared<EndState>(
                            ResourceManager::getTexture("background"), m_windowInfo, m_mouse, m_currentScore
                        )
                    );
            }
        }
    ...
    case StateName::Selection:
        {
            if(std::shared_ptr<SpaceshipSelectionState> selection
               = std::dynamic_pointer_cast<SpaceshipSelectionState>(m_states.top()))
            {
    ...
    case StateName::End:
        {
            if(std::shared_ptr<EndState> end = std::dynamic_pointer_cast<EndState>(m_states.top()))
            {
                if(end->getBackButtonPressed())
                {
                    m_states.pop();
                    m_states.push(
                        std::make_shared<MenuState>(ResourceManager::getTexture("background"), m_windowInfo, m_mouse)
                    );
...
{
    glm::mat4 projection{
        glm::ortho(0.f, static_cast<float>(m_windowInfo->width), static_cast<float>(m_windowInfo->height), 0.f)
    };
