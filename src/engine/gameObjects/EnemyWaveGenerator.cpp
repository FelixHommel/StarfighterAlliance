...
EnemyWaveGenerator::EnemyWaveGenerator(const Texture2D& textureMeteorite, const Texture2D& textureAlien, const WindowInfo* windowInfo)
...
void EnemyWaveGenerator::update(float  dt)
{
    ...
    if(m_activeWaves.empty() || m_activeWaves.back().getWavePositionY() - (m_activeWaves.back().getEnemyHeight() + EnemyWave::startHeight) > minGap)
        spawnWave();
    ...
    for(size_t i{0}; i < m_activeWaves.size(); ++i)
        if(!m_activeWaves[i].isActive() || m_activeWaves[i].getWavePositionY() > static_cast<float>(m_windowInfo->height))
            killWave(i);
}
