...
Spaceship::Spaceship(SpaceshipType type, ColorType color, const WindowInfo* windowInfo)
    ...
    , defaultPosition(glm::vec2((static_cast<float>(m_windowInfo->width) * 0.5f) - (static_cast<float>(m_windowInfo->width) * 0.05f),
				static_cast<float>(m_windowInfo->height) - (static_cast<float>(m_windowInfo->height) * 0.15f)))
    , defaultSize(glm::vec2(static_cast<float>(m_windowInfo->width) * 0.1f, static_cast<float>(m_windowInfo->height) * 0.1f))
	, m_type(type)
	, m_speed(0)
	, m_shotPower(0.f)
    , m_shot(false)
{
    ...
    m_shooting = std::make_shared<ShootingComponent>(ResourceManager::getTexture("Projectile"), &m_position, m_shotPower, m_windowInfo);
}
...
void Spaceship::update(float dt)
{
    ...
    if(glfwGetKey(m_windowInfo->window.get(), GLFW_KEY_W) == GLFW_PRESS || glfwGetKey(m_windowInfo->window.get(), GLFW_KEY_UP) == GLFW_PRESS)
        m_movement->moveY(-m_speed * movementSpeedFactor * dt);
    else if(glfwGetKey(m_windowInfo->window.get(), GLFW_KEY_S) == GLFW_PRESS || glfwGetKey(m_windowInfo->window.get(), GLFW_KEY_DOWN) == GLFW_PRESS)
        m_movement->moveY(m_speed * movementSpeedFactor * dt);

    if(glfwGetKey(m_windowInfo->window.get(), GLFW_KEY_A) == GLFW_PRESS || glfwGetKey(m_windowInfo->window.get(), GLFW_KEY_LEFT) == GLFW_PRESS)
        m_movement->moveX(-m_speed * movementSpeedFactor * dt);
    else if(glfwGetKey(m_windowInfo->window.get(), GLFW_KEY_D) == GLFW_PRESS || glfwGetKey(m_windowInfo->window.get(), GLFW_KEY_RIGHT) == GLFW_PRESS)
        m_movement->moveX(m_speed * movementSpeedFactor * dt);
...
void Spaceship::readDataFromFile()
{
	...
	if(file.open(filepath.str()))
    {
		...
		throw std::runtime_error("[ERROR] could not open file or file is invalid.\n Path of the file: " + filepath.str());
}
...
{
	std::stringstream filepath;
	filepath << "resources/textures/" << spaceshipTypeToString(m_type) << "/" << spaceshipTypeToString(m_type) << "_" << Color::toString(color) << ".png";
...
std::string Spaceship::spaceshipTypeToString(SpaceshipType type)
{
	switch(type)
    {
        case SpaceshipType::x_wing:			  return "x_wing";
        case SpaceshipType::y_wing:			  return "y_wing";
        case SpaceshipType::millenium_falcon: return "millenium_falcon";
    }
}
