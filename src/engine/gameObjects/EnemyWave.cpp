    ...
    , m_isActive(true)
{
	...
	std::seed_seq ss {static_cast<std::seed_seq::result_type>(std::chrono::steady_clock::now().time_since_epoch().count()), rd(), rd(), rd()};
...
void EnemyWave::createWave()
{
    ...
    while(!finished)
    {
        ...
        if(nextX + offset + (static_cast<float>(m_windowInfo->height) * width) < static_cast<float>(m_windowInfo->width))
        {
            if(random == 1)
                m_enemies.push_back(Alien(m_textureAlien, glm::vec2(nextX + offset, startHeight), width, m_windowInfo));
            else 
                m_enemies.push_back(Meteorite(m_textureMeteorite, type, glm::vec2(nextX + offset, startHeight), width, m_windowInfo));
...
std::pair<float, float> EnemyWave::getWidthRange(MeteoriteType type)
{
    switch(type)
    {
        case MeteoriteType::SMALL:	return std::make_pair(0.05f, 0.1f);
        case MeteoriteType::MEDIUM: return std::make_pair(0.1f, 0.15f);
        case MeteoriteType::BIG:	return std::make_pair(0.15f, 0.2f);
    }
}
...
void EnemyWave::checkForBigEnoughHole(bool& flag)
{
    ...
    {
        if(m_enemies[i + 1].getPosition().x - (m_enemies[i].getPosition().x + m_enemies[i].getSize().x) > spaceshipWidth)
        {
            flag = true;
            return;
        }
