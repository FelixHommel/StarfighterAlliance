...
SpaceshipSelectionState::SpaceshipSelectionState(const Texture2D& background, const WindowInfo* windowInfo, const Mouse* mouse)
...
{
	m_startButton = new Button(glm::vec2(m_windowInfo->width * 0.4, m_windowInfo->height * 0.85),
		glm::vec2(m_windowInfo->width * 0.2, m_windowInfo->height * 0.1), glm::vec3(158.f / 255.f, 80.f / 255.f, 250.f / 255.f),
		glm::vec3(100.f / 255.f, 33.f / 255.f, 177.f / 255.f), glm::vec3(81.f / 255.f, 8.f / 255.f, 166.f / 255.f), "Start",
		ResourceManager::getShader("button"), 0.5f);
...
void SpaceshipSelectionState::render(SpriteRenderer& renderer, TextRenderer& textRenderer)
{
	...
	renderer.draw(m_spaceships.at(static_cast<int>(m_selectedType))->texture, {m_windowInfo->width * 0.4f, m_windowInfo->height * 0.4f},
    ...
    textRenderer.draw(std::to_string(m_spaceships.at(static_cast<int>(m_selectedType))->speed), m_windowInfo->width * 0.125f, m_windowInfo->height * 0.715f);
    textRenderer.draw(std::to_string(m_spaceships.at(static_cast<int>(m_selectedType))->lives), m_windowInfo->width * 0.865f, m_windowInfo->height * 0.715f);
    textRenderer.draw(std::to_string(m_spaceships.at(static_cast<int>(m_selectedType))->shotPower), m_windowInfo->width * 0.75f, m_windowInfo->height * 0.2f);
...
void SpaceshipSelectionState::loadSpaceshipData()
{
    ...
    for(int i {0}; i < m_spaceships.size(); ++i)
    {
        ...
        m_spaceships.at(i) = std::make_shared<SpaceshipData>(
            lives.value(), speed.value(), shotPower.value(), ResourceManager::loadTexture(ssPath.str().c_str(), ssName.str()));
    }
}
